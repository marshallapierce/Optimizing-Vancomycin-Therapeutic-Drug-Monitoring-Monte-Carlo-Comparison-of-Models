...
pop_params = {}
param_names = ['Vc', 'Vp', 'Cl_total', 'Cl_dist']
for param in param_names:
    while True:
        try:
            value = float(input(f"Enter two-compartment population mean {param} (positive non-zero number): "))
            if value <= 0:
                raise ValueError("Value must be positive and non-zero.")
            pop_params[param] = value
            break
        except ValueError as e:
            print(f"Invalid input for {param}: {e}. Please try again.")
...
for param in param_names:
    while True:
        try:
            value = float(input(f"Enter two-compartment Coefficient of variation for {param} CV=SD/Mean (positive non-zero number): "))
            if value <= 0:
                raise ValueError("Value must be positive and non-zero.")
            cvs[param] = value
            break
        except ValueError as e:
            print(f"Invalid input for CV of {param}: {e}. Please try again.")

# User input for dose interval Tau
while True:
    try:
        Tau = float(input("Enter the dose interval (Tau) in hours: "))
        if Tau <= 0:
            raise ValueError("Tau must be positive and non-zero.")
        break
    except ValueError as e:
        print(f"Invalid input for Tau: {e}. Please try again.")

# User input for dose (mg)
# while True:
#     try:
#         Dose = float(input("Enter the dose (mg): "))
#         if Dose <= 0:
#             raise ValueError("Dose must be positive and non-zero.")
#         break
#     except ValueError as e:
#         print(f"Invalid input for Dose: {e}. Please try again.")
Dose=450.0*pop_params['Cl_total']*Tau/24  # Dose to target AUC of 450 based on population mean Cl_total

# User input for infusion period (hours)
while True:
    try:
        InfusionPeriod = float(input("Enter the infusion period (hours): "))
        if InfusionPeriod <= 0:
            raise ValueError("Infusion period must be positive and non-zero.")
        break
    except ValueError as e:
        print(f"Invalid input for InfusionPeriod: {e}. Please try again.")

# Calculate limits as mean +/- 2 SD
limits = {}
for param in param_names:
    mean = pop_params[param]
    sd = mean * cvs[param]
    lower = mean - 2 * sd
    upper = mean + 2 * sd
    limits[param] = (lower, upper)


N = 30000
# the times below are the acceptable times for levels to be entered, from 0 to Tau - InfusionPeriod in hourly increments.
times_true = np.arange(0, Tau - InfusionPeriod + 1, 1)# User entered time levels for two compartment fitting, levels in grids.cvs

# User input for fitting times (up to 8)
times_fit_input = input(f"Bayesian Analysis One compartment model: Enter up to 8 post dose.times separated by space or comma (available: 0 to {int(Tau - InfusionPeriod)} hours): ").replace(',', ' ').split()
try:
...
def calculate_cp_2comp(Vc, Vp, Cl_total, Cl_dist, times):
    ...
    beta = 0.5 * (sum_k - disc)
    A = Dose * (K21 - alpha) * (1 - np.exp(-alpha * InfusionPeriod)) / (InfusionPeriod * Vc * alpha * (beta - alpha) * (1 - np.exp(-alpha * Tau)))
    B = Dose * (beta - K21) * (1 - np.exp(-beta * InfusionPeriod)) / (InfusionPeriod * Vc * beta * (beta - alpha) * (1 - np.exp(-beta * Tau)))
...
AUC_true = Dose * (24 / Tau) / params['Cl_total'] # Calculates AUC based on true Cl_total randomized values
...
AUCcalc = np.zeros(N)
for i in range(N):
    idx_peak = np.where(times_true == 2)[0][0] # Index of peak time point (2 hours after end of infusion)
    idx_trough = np.where(times_true == Tau - InfusionPeriod)[0][0] # Index of trough time point
    interval = Tau - InfusionPeriod - 2
    Kcalc = np.log(randomized_levels[i, idx_peak] / randomized_levels[i, idx_trough]) / interval
    Vdcalc[i] = Dose * (1 - np.exp(-Kcalc * InfusionPeriod)) * np.exp(-Kcalc * 2) / (randomized_levels[i, idx_peak] * Kcalc * InfusionPeriod * (1 - np.exp(-Kcalc * Tau)))
    Clcalc[i] = Kcalc * Vdcalc[i]
    AUCcalc[i] = Dose * (24 / Tau) / Clcalc[i]

Vdpop_mean = np.mean(Vdcalc[Vdcalc > 0])
...
# User input for Vdpop selection
vdpop_options = {
    'mean': Vdpop_mean,
    'median': Vdpop_median,
    'geometric_mean': Vdpop_geometric_mean
}
while True:
    choice = input("Select basis of Vdpop in one-compartment model (mean, median, geometric_mean): ").strip().lower()
    if choice in vdpop_options:
        Vdpop = vdpop_options[choice]
        break
    else:
        print("Invalid choice. Please enter mean, median, or geometric_mean.")
...
def calculate_cp_1comp_fixedVd(Cl, times, Vd=Vdpop):
    K = Cl / Vd
    Cp = Dose * (1 - np.exp(-K * InfusionPeriod)) * np.exp(-K * times) / (Cl * InfusionPeriod * (1 - np.exp(-K * Tau)))
    return Cp
...
trough_time = Tau - InfusionPeriod
for i in range(N):
    idx_trough = np.where(times_true == trough_time)[0][0]
    target = randomized_levels[i, idx_trough]
    def obj(Cl):
        return (calculate_cp_1comp_fixedVd(Cl, trough_time) - target)**2
...
AUC_fit_fixedVd = Dose * (24 / Tau) / fitted_Cl_fixedVd
...
AUC_fit_bayes = Dose * (24 / Tau) / fitted_Cl_bayes

# Save all data to one CSV
with open(f'monte_carlo_all_data_onevrstwocompartment_geometric_mean{file_suffix}.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    header = (['Group'] + 
...
with open(f'grid_onevrstwocompart_Geometric_Mean{file_suffix}.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    for model, hist, auc_fit in zip(models, hists, auc_fits):
...
print(f"Grids saved to grid_onevrstwocompart_Geometric_Mean{file_suffix}.csv")
...
with open(f'cl_differences_statistics_Geometric_Mean{file_suffix}.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    ...
    writer.writerow(['Vdpop Mean', Vdpop_mean])
    writer.writerow(['Clpop', Clpop])
    writer.writerow(['Vdpop Mean Std Dev', Vdpop_std_mean])
...
print(f"Statistics saved to cl_differences_statistics_Geometric_Mean{file_suffix}.csv")