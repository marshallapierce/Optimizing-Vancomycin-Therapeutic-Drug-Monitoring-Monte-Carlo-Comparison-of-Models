...
# Population parameters for two-compartment
base_pop_params = {'Vc': 58.4, 'Vp': 38.4, 'Cl_total': 4.5, 'Cl_dist': 6.5}
cvs = {'Vc': 0.3, 'Vp': 0.3, 'Cl_total': 0.3, 'Cl_dist': 0.4}
base_limits = {'Vc': (23, 93), 'Vp': (15, 61), 'Cl_total': (1.8, 7.2), 'Cl_dist': (1.3, 11.7)}
...
# Cl_total means for different renal function levels
Cl_total_means = [4.5, 3.6, 2.6, 1.5]#change these values to simulate different renal function levels, change the dose too

for cl_mean in Cl_total_means:
    print(f"\nRunning simulation for Cl_total mean = {cl_mean}", flush=True)
    np.random.seed(42)
    
    # Set parameters for this renal function level
    pop_params = base_pop_params.copy()
    pop_params['Cl_total'] = cl_mean
    sd_cl = cl_mean * cvs['Cl_total']
    limits = base_limits.copy()
    limits['Cl_total'] = (max(0, cl_mean - 2 * sd_cl), cl_mean + 2 * sd_cl)
    fit_bounds = {'Vc': (5.8, 110), 'Vp': (3.8, 72), 'Cl_total': (max(0.1, cl_mean - 3 * sd_cl), cl_mean + 3 * sd_cl), 'Cl_dist': (1.3, 11.7)}
...
def calculate_cp_2comp(Vc, Vp, Cl_total, Cl_dist, times):
    ...
    if disc <= 0 or np.isnan(disc):
        return np.full(len(times), np.nan)
    alpha = 0.5 * (sum_k + disc)
    beta = 0.5 * (sum_k - disc)
    if alpha == beta or np.isnan(alpha) or np.isnan(beta):
        return np.full(len(times), np.nan)
...
AUC_true = 1012.6 * (24 / 12) / params['Cl_total'] # Calculates AUC based on true Cl_total randomized values
...
noise = np.random.normal(0, 0.1, (N, len(times_true)))
randomized_levels = true_levels * (1 + noise)
randomized_levels = np.clip(randomized_levels, 0.01, None)  # Avoid negative or zero
...
for i in range(N):
    ...
    ratio = randomized_levels[i, idx_2h] / randomized_levels[i, idx_10h]
    if ratio > 1 and not np.isnan(ratio):
        Kcalc = np.log(ratio) / 8
        Vdcalc[i] = 1012.6 * (1 - np.exp(-Kcalc * 2)) * np.exp(-Kcalc * 2) / (randomized_levels[i, idx_2h] * Kcalc * 2 * (1 - np.exp(-Kcalc * 12)))
        Clcalc[i] = Kcalc * Vdcalc[i]
        AUCcalc[i] = 1012.6 * (24 / 12) / Clcalc[i]
    else:
        Vdcalc[i] = np.nan
        Clcalc[i] = np.nan
        AUCcalc[i] = np.nan

valid = ~np.isnan(Vdcalc) & ~np.isnan(Clcalc) & (Vdcalc > 0) & (Clcalc > 0)
if np.any(valid):
    Vdpop = np.mean(Vdcalc[valid])
    Clpop = np.mean(Clcalc[valid])
else:
    Vdpop = 100  # default
    Clpop = 3
...
def calculate_cp_1comp_fixedVd(Cl, times, Vd=Vdpop):
    K = Cl / Vd
    Cp = 1012.6 * (1 - np.exp(-K * 2)) * np.exp(-K * times) / (Cl * 2 * (1 - np.exp(-K * 12)))
    return Cp
...
for i in range(N):
    ...
    if np.isnan(target) or np.any(np.isnan(selected_levels[i]) | np.isinf(selected_levels[i])):
        ...
        try:
            res = optimize.minimize_scalar(obj, bounds=bounds, method='bounded', options={'maxiter':50})
            ...
            print(f"Error in fixed Vd fit for i={i}: {e}")
            fitted_Cl_fixedVd[i] = np.nan
            fitted_levels_fixedVd[i] = np.full(len(times_fit), np.nan)
            sse_fixedVd[i] = np.nan

AUC_fit_fixedVd = 1012.6 * (24 / 12) / fitted_Cl_fixedVd
...
def residuals_1comp(p, times, levels):
    if np.any(np.isnan(levels)) or np.any(np.isnan(p)):
        return np.full(len(levels) + 2, 1e10)
    Vd, Cl = p
...
for i in range(N):
    if np.any(np.isnan(selected_levels[i]) | np.isinf(selected_levels[i])):
        ...
        try:
            res = optimize.least_squares(residuals_1comp, x0, bounds=bounds, args=(selected_times, selected_levels[i]), max_nfev=10, ftol=1e-2, xtol=1e-2)
            ...
            print(f"Error in bayesian fit for i={i}: {e}")
            fitted_Vd_bayes[i] = np.nan
            fitted_Cl_bayes[i] = np.nan
            fitted_levels_bayes[i] = np.full(len(times_fit), np.nan)
            sse_bayes[i] = np.nan

AUC_fit_bayes = 1012.6 * (24 / 12) / fitted_Cl_bayes

# Save all data to one CSV
print(f"Completed fits for Cl = {cl_mean}")
with open(f'monte_carlo_all_data_onevrstwocompartment_Cl{cl_mean}.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    header = (['Group'] + 
    ...
    print(f"Results saved to monte_carlo_all_data_onevrstwocompartment_Cl{cl_mean}.csv", flush=True)
...
with open(f'grid_onevrstwocompart_Cl{cl_mean}.csv', 'w', newline='') as csvfile:
    ...
    print(f"Grids saved to grid_onevrstwocompart_Cl{cl_mean}.csv", flush=True)
...
aic_fixed = 2 * k_fixed + n * np.log(np.maximum(sse_fixedVd, 1e-10) / n)
aic_bayes = 2 * k_bayes + n * np.log(np.maximum(sse_bayes, 1e-10) / n)

avg_aic_fixed = np.mean(aic_fixed)
avg_aic_bayes = np.mean(aic_bayes)

# Save statistics to separate CSV
with open(f'cl_differences_statistics_Cl{cl_mean}.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    ...
    print(f"Statistics saved to cl_differences_statistics_Cl{cl_mean}.csv", flush=True)
    print(f"Finished for Cl = {cl_mean}", flush=True)
