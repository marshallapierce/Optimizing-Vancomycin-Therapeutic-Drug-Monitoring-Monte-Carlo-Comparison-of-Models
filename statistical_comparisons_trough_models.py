...
def create_auc_comparison_csv(auc_true, auc_models, model_names, suffix):
    ...
    for auc_pred, name in zip(auc_models, model_names):
        ...
        lines.append("")
    # Write to file
    filename = f'{output_dir}/auc_comparison_grids_troughs_trough_vr_trough_combined.csv'
...
def main():
    ...
    results_file = open(f'{output_dir}/statistical_comparison_results_trough_vr_trough_combined.txt', 'w')
    ...
    try:
        if df_one is not None and df_two is not None:
            ...
            auc_bayes = df['AUC_fit_bayes_last'].values  # 1-compt Bayesian trough
            auc_fixed = df['AUC_fit_fixed'].values  # 1-compt Fixed VD trough
            auc_fit_two = df['AUC_fit'].values  # 2-compt Bayesian trough
            ...
            bland_altman_plot(auc_true, auc_bayes, 'Bland-Altman: One-Compartment Bayesian Trough Model AUC vs True AUC', f'merged_output/bland_altman_one_compt_bayes_trough_vr_trough_combined.png')
            bland_altman_plot(auc_true, auc_fit_two, 'Bland-Altman: Two-Compartment Bayesian Trough Model AUC vs True AUC', f'merged_output/bland_altman_two_compt_bayes_trough_vr_trough_combined.png')
            bland_altman_plot(auc_true, auc_fixed, 'Bland-Altman: One-Compartment Fixed VD Trough Model AUC vs True AUC', f'merged_output/bland_altman_one_compt_fixed_trough_vr_trough_combined.png')
            
            cl_true = df['Cl_total_true_one'].values
            cl_calc = df['Clcalc'].values
            cl_bayes = df['Cl_fit_bayes_last'].values
            cl_fixed = df['Cl_fit_fixed'].values
            cl_fit_two = df['Cl_total_fit'].values
            
            cp_10_bayes = df['Cp_10.0_fit_bayes_last'].values  # 1-compt Bayesian trough
            ...
            print_and_save(f"=== Trough-Based Pharmacokinetic Model Comparisons (Combined Data) ===", results_file)
            ...
            print_and_save("\n=== 1-Compartment Bayesian Trough vs 2-Compartment Bayesian Trough ===", results_file)
        ...
        print_and_save(f"AUC Diff: 1-Compt Bayes = {np.mean(auc_diff_1_bayes):.2f}, 2-Compt Bayes = {np.mean(auc_diff_2):.2f}", results_file)
        print_and_save(f"Paired t-test: t = {t_auc_12:.3f}, {format_p_value(p_auc_12)} (Bonferroni corrected: {format_p_value(p_auc_12_corrected)})", results_file)
        ...
        print_and_save(f"AUC 95% CI (1-Compt Bayes): ({ci_lower_1:.2f}, {ci_upper_1:.2f})", results_file)

        ci_lower_2 = np.mean(auc_diff_2) - 1.96 * np.std(auc_diff_2, ddof=1)
        ci_upper_2 = np.mean(auc_diff_2) + 1.96 * np.std(auc_diff_2, ddof=1)
        print_and_save(f"AUC 95% CI (2-Compt Bayes): ({ci_lower_2:.2f}, {ci_upper_2:.2f})", results_file)

        # RMSE calculations
        auc_rmse_1_bayes = np.sqrt(np.mean(auc_diff_1_bayes**2))
        auc_rmse_2 = np.sqrt(np.mean(auc_diff_2**2))
        print_and_save(f"AUC RMSE: 1-Compt Bayes = {auc_rmse_1_bayes:.2f}, 2-Compt Bayes = {auc_rmse_2:.2f}", results_file)
        ...
        print_and_save(f"AUC Percentage Error Mean: 1-Compt Bayes = {mean_perc_auc_1_bayes:.2f}%, 2-Compt Bayes = {mean_perc_auc_2:.2f}%", results_file)
        print_and_save(f"AUC Percentage Error RMSE: 1-Compt Bayes = {rmse_perc_auc_1_bayes:.2f}%, 2-Compt Bayes = {rmse_perc_auc_2:.2f}%", results_file)
        ...
        print_and_save(f"McNemar test: statistic = {result_mc.statistic:.3f}, {format_p_value(result_mc.pvalue)} (Bonferroni corrected: {format_p_value(p_mc_corrected)})", results_file)
        print_and_save(f"Contingency table:\n{table_mc}", results_file)
        correct_1_compt = int(table_mc[1, 0] + table_mc[1, 1])
        correct_2_compt = int(table_mc[0, 1] + table_mc[1, 1])
        print_and_save(f"Correct classifications: 1-Compt Bayes = {correct_1_compt}, 2-Compt Bayes = {correct_2_compt}", results_file)
        ...
        print_and_save(f"Cl Diff: 1-Compt Bayes = {np.mean(cl_diff_1_bayes):.2f}, 2-Compt Bayes = {np.mean(cl_diff_2):.2f}", results_file)
        print_and_save(f"Paired t-test: t = {t_cl_12:.3f}, {format_p_value(p_cl_12)} (Bonferroni corrected: {format_p_value(p_cl_12_corrected)})", results_file)
        ...
        print_and_save(f"Cl 95% CI (1-Compt Bayes): ({ci_cl_lower_1:.2f}, {ci_cl_upper_1:.2f})", results_file)

        ci_cl_lower_2 = np.mean(cl_diff_2) - 1.96 * np.std(cl_diff_2, ddof=1)
        ci_cl_upper_2 = np.mean(cl_diff_2) + 1.96 * np.std(cl_diff_2, ddof=1)
        print_and_save(f"Cl 95% CI (2-Compt Bayes): ({ci_cl_lower_2:.2f}, {ci_cl_upper_2:.2f})", results_file)

        # Cl RMSE
        cl_rmse_1_bayes = np.sqrt(np.mean(cl_diff_1_bayes**2))
        cl_rmse_2 = np.sqrt(np.mean(cl_diff_2**2))
        print_and_save(f"Cl RMSE: 1-Compt Bayes = {cl_rmse_1_bayes:.2f}, 2-Compt Bayes = {cl_rmse_2:.2f}", results_file)
        ...
        print_and_save(f"Trough Cp: 1-Compt Bayes = {np.mean(cp_10_bayes):.2f}, 2-Compt Bayes = {np.mean(cp_10_fit_two):.2f}", results_file)
        print_and_save(f"Paired t-test: t = {t_cp_12:.3f}, {format_p_value(p_cp_12)} (Bonferroni corrected: {format_p_value(p_cp_12_corrected)})", results_file)

        # Section 2: 1-Compartment Fixed VD with Trough vs 2-Compartment Bayesian with Trough
        print_and_save("\n=== 1-Compartment Fixed VD Trough vs 2-Compartment Bayesian Trough ===", results_file)
        ...
        print_and_save(f"AUC Diff: Fixed VD = {np.mean(auc_diff_fixed):.2f}, 2-Compt Bayes = {np.mean(auc_diff_2):.2f}", results_file)
        print_and_save(f"Paired t-test: t = {t_auc_f2:.3f}, {format_p_value(p_auc_f2)} (Bonferroni corrected: {format_p_value(p_auc_f2_corrected)})", results_file)
        ...
        print_and_save(f"AUC 95% CI (Fixed VD): ({ci_lower_fixed:.2f}, {ci_upper_fixed:.2f})", results_file)
        print_and_save(f"AUC 95% CI (2-Compt Bayes): ({ci_lower_2:.2f}, {ci_upper_2:.2f})", results_file)

        # RMSE calculations
        auc_rmse_fixed = np.sqrt(np.mean(auc_diff_fixed**2))
        print_and_save(f"AUC RMSE: Fixed VD = {auc_rmse_fixed:.2f}, 2-Compt Bayes = {auc_rmse_2:.2f}", results_file)
        ...
        print_and_save(f"AUC Percentage Error Mean: Fixed VD = {mean_perc_auc_fixed:.2f}%, 2-Compt Bayes = {mean_perc_auc_2:.2f}%", results_file)
        print_and_save(f"AUC Percentage Error RMSE: Fixed VD = {rmse_perc_auc_fixed:.2f}%, 2-Compt Bayes = {rmse_perc_auc_2:.2f}%", results_file)

        # Pearson's r for AUC
        print_and_save(f"AUC Pearson's r: Fixed VD = {pearson_r_fixed:.3f}, 2-Compt Bayes = {pearson_r_two:.3f}", results_file)

        # McNemar test
        result_mc_f, table_mc_f = mcnemar_test(auc_true, auc_fixed, auc_fit_two)
        p_mc_f_corrected = min(result_mc_f.pvalue * 8, 1.0)
        print_and_save(f"McNemar test: statistic = {result_mc_f.statistic:.3f}, {format_p_value(result_mc_f.pvalue)} (Bonferroni corrected: {format_p_value(p_mc_f_corrected)})", results_file)
        print_and_save(f"Contingency table:\n{table_mc_f}", results_file)
        correct_fixed = int(table_mc_f[1, 0] + table_mc_f[1, 1])
        correct_2_compt_f = int(table_mc_f[0, 1] + table_mc_f[1, 1])
        print_and_save(f"Correct classifications: Fixed VD = {correct_fixed}, 2-Compt Bayes = {correct_2_compt_f}", results_file)

        # Cl comparison
        cl_diff_fixed = cl_fixed - cl_true
        t_cl_f2, p_cl_f2 = stats.ttest_rel(cl_diff_fixed, cl_diff_2)
        p_cl_f2_corrected = min(p_cl_f2 * 8, 1.0)
        print_and_save(f"Cl Diff: Fixed VD = {np.mean(cl_diff_fixed):.2f}, 2-Compt Bayes = {np.mean(cl_diff_2):.2f}", results_file)
        print_and_save(f"Paired t-test: t = {t_cl_f2:.3f}, {format_p_value(p_cl_f2)} (Bonferroni corrected: {format_p_value(p_cl_f2_corrected)})", results_file)
        ...
        print_and_save(f"Cl 95% CI (Fixed VD): ({ci_cl_lower_fixed:.2f}, {ci_cl_upper_fixed:.2f})", results_file)
        print_and_save(f"Cl 95% CI (2-Compt Bayes): ({ci_cl_lower_2:.2f}, {ci_cl_upper_2:.2f})", results_file)

        # Cl RMSE
        cl_rmse_fixed = np.sqrt(np.mean(cl_diff_fixed**2))
        print_and_save(f"Cl RMSE: Fixed VD = {cl_rmse_fixed:.2f}, 2-Compt Bayes = {cl_rmse_2:.2f}", results_file)

        # Trough concentration comparison
        t_cp_f2, p_cp_f2 = stats.ttest_rel(cp_10_fixed, cp_10_fit_two)
        p_cp_f2_corrected = min(p_cp_f2 * 8, 1.0)
        print_and_save(f"Trough Cp: Fixed VD = {np.mean(cp_10_fixed):.2f}, 2-Compt Bayes = {np.mean(cp_10_fit_two):.2f}", results_file)
        print_and_save(f"Paired t-test: t = {t_cp_f2:.3f}, {format_p_value(p_cp_f2)} (Bonferroni corrected: {format_p_value(p_cp_f2_corrected)})", results_file)
